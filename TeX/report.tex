...
The ES layer is a basic logic reasoner. It is not aware of its current reality, or any context. It simply has a list of known Facts, in the form of tags, and outputs tags based on known relationships, called Rules. These output tags are passed on to the final layer, the Meta Reasoner (META).
...
\begin{figure*}[!htb]
	...
	\caption{Setup for the \code{testKNN()} method. Every node has a \code{threshold} value of 1 to simplify the activation.}
	\label{testKNN}
\end{figure*}
...
\begin{table*}
	...
	\end{tabular}
	\caption{Test setup for \code{testES()}. Middle activation steps omitted.}
	\label{testES}
\end{table*}
