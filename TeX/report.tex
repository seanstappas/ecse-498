...
% TODO: source (benefit of convolutional neural networks)
...
% TODO: source (humans classify with uncertainty)
...
% TODO: source other than Vybihal (background thinking)
...
% TODO: source (thinking effort)

There is a strength value associated with every Knowledge Node which represents how much weight an activation has and therefore how quickly that node will fire. A simple implementation of strength would be as a constant coefficient multiplying each activation increment. So, instead of checking when the activation is greater than the threshold, one would check if $activation * strength \geq threshold$.

Confidence was mentioned earlier in the context of thinking backwards. This concept can however extend beyond this context. Indeed, every individual Knowledge Node can have a confidence value associated with it, representing how certain the KNN is that the input tag is true. These confidence values can come from the NN, since a neural network will always have a confidence value when classifying objects. When initiating a think cycle, the confidence value at each stage can be multiplied with each other, making the KNN less certain of a memory as it searches through its tree of Knowledge Nodes. This represents memories in the human brain require a great deal of thinking to reach and, as such, are less certain than other memories.
% TODO: source (certainty of memories)
...
% TODO: Maybe include schedule (GANTT chart)
...
% TODO: source (AI in nuclear disaster)
